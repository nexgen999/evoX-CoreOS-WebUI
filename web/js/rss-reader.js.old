...
class EvoXChangelogReader {
    ...
    async init(changelogUrl, containerId) {
        ...
        try {
            // Lecture du fichier CHANGELOG.md (avec fallback GitHub Pages / Raw)
            ...
            if (!rawMd) {
                // Si pas de fichier distant, essaye le fichier local
    ...
    parseChangelog(mdText) {
        // Découpage du markdown par versions (recherche des titres # ou ##)
        ...
        sections.forEach(sec => {
            ...
            // Extraction version et date si présent (ex: [1.2.0] - 2026-05-20)
