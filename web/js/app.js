...
let cfg, repo, profile, repository, storeData, newsData, homeNewsData, docsData=[];
...
function socialsUI(){
  ...
  const top=(cfg.topbar?.showSocials!==false?list:[]);
  const quick=(cfg.topbar?.quickDownloads||[]).filter(x=>x.enabled!==false&&x.url);
  const quickHtml=quick.map(x=>`<a class="top-social top-download" href="${esc(resolveUrl(x.url,repo))}" target="_blank" rel="noopener" title="${esc(x.label)}">${iconHtml(x.icon||"download")}<span>${esc(x.showLabel===false?"":x.label)}</span></a>`).join("");
  const socialHtml=top.map(s=>`<a class="top-social" href="${esc(s.url||repo.githubUrl||"#")}" target="_blank" rel="noopener" title="${esc(s.label)}">${s.iconType==="image"?`<img src="${esc(resolveUrl(s.icon,repo))}" alt="">`:iconHtml(s.icon||"link")}<span>${esc(s.showLabel? s.label : "")}</span></a>`).join("");
  const avatar=profile?.avatar_url||cfg.profile?.avatar||"./web/assets/avatar-placeholder.svg";
  $("#top-socials").innerHTML=quickHtml+socialHtml+`<img class="top-avatar" src="${esc(avatar)}" alt="">`;
  const promo=cfg.sidebar?.promo||{};
  $("#sidebar-bottom").innerHTML=`${promo.enabled===false?"":`<div class="sidebar-promo"><div class="sidebar-promo-art"></div><strong>${esc(promo.title||cfg.site.name||"evoX")}</strong><span>${esc(promo.text||"One place. All your needs.")}</span></div>`}`;
}
function shell(){nav();profileUI();socialsUI();$("#global-brand-logo").src=cfg.branding.logo||"./web/assets/logo.svg";$("#global-brand-title").textContent=cfg.site.shortName||cfg.site.name;$("#global-brand-subtitle").textContent=(cfg.site.name||"").replace(cfg.site.shortName||"","").trim()||"Core OS";$("#github-top").href=repo.githubUrl||"#";$("#github-top").style.display=repo.githubUrl?"":"none"}
function hero(){
...
async function home(){
 setTitle("Home");
 if(!homeNewsData) homeNewsData=await readAllFeeds(cfg, cfg.homeNews||cfg.news||{});
 const latest=(homeNewsData?.items||[]).slice(0,4);
 const aio=(cfg.home?.quickDownloads||cfg.downloads?.items||[]).slice(0,4).map(x=>[x.name,x.description||"Pack complet (.zip)",x.url]);
 const cats=[...new Set((storeData?.items||[]).map(x=>x.category).filter(Boolean))].sort();
 const countFor=c=> (storeData?.items||[]).filter(x=>x.category===c).length;
 const configuredCats=(cfg.home?.categoryTiles||[]).filter(x=>x.enabled!==false);
 const categoryNames=(configuredCats.length?configuredCats.map(x=>x.label):cats.length?cats:["Payloads","Packages","FFPFSC","Applications","Tools","Divers"]).slice(0,6);
 const categoryIcons=(configuredCats.length?configuredCats.map(x=>x.icon||"✦"): ["✦","◆","⚒","▣","⌘","◇"]).slice(0,6);
 content.innerHTML=`
 <section class="approved-hero">
   <div class="hero-art"><img src="${esc(cfg.branding.logo||'./web/assets/logo.svg')}" alt="evoX Core OS"></div>
   <div class="hero-copy">
     <div class="eyebrow">${esc(cfg.site.name||'evoX CORE OS')}</div>
     <h1>${esc(cfg.branding.heroTitle||'Bienvenue sur evoX Core OS')}</h1>
     <p>${esc(cfg.branding.heroText||'Un espace unique pour accéder à mes projets, ressources, news et bien plus encore. Explore, télécharge, découvre !')}</p>
     <div class="hero-actions">${(cfg.home?.heroLinks||[
       {label:"Payloads PS5",url:"#store",icon:"✦"},{label:"Store",url:"#store",icon:"▣"},{label:"News RSS",url:"#news",icon:"◔"},{label:"Documentation",url:"#docs",icon:"▤"},{label:"Open Source",url:repo.githubUrl||"#",icon:"◈"}
     ]).map(x=>`<a class="hero-chip" href="${esc(x.url || (x.label==='Open Source'?repo.githubUrl:'#'))}" ${/^https?:/i.test(x.url || (x.label==='Open Source'?repo.githubUrl:''))?'target="_blank" rel="noopener"':''}>${esc(x.icon||"")} ${esc(x.label)}</a>`).join("")}</div>
   </div>
 </section>
 <div class="approved-columns">
   <section class="approved-news panel">
    <div class="approved-head"><div><h2>◔ &nbsp;Dernières News</h2><span>Actualités, mises à jour et infos du moment</span></div><a class="outline-btn" href="#news">Voir toutes les news&nbsp; →</a></div>
    <div class="approved-news-list">${latest.length?latest.map(n=>`<article class="approved-news-item"><div class="approved-thumb">${n.image?`<img src="${esc(n.image)}" alt="">`:`<span>PS5</span>`}</div><div class="approved-news-copy"><h3>${esc(n.title)}</h3><div class="approved-meta"><span>${esc(n.source||'RSS')}</span><span>◷ ${esc(fmtDate(n.date))}</span></div><p>${esc(n.description||'').slice(0,190)}${(n.description||'').length>190?'…':''}</p><a href="${esc(n.link||'#')}" target="_blank" rel="noopener">Lire la suite →</a></div></article>`).join(''):`<div class="empty">Les flux RSS / OPML apparaîtront ici automatiquement.</div>`}</div>
   </section>
   <div class="approved-right">
    <section class="approved-welcome panel"><div class="welcome-art"></div><div class="approved-welcome-content"><h2>${esc(cfg.home?.welcomeTitle||"👋 Bienvenue !")}</h2><p>${esc(cfg.home?.welcomeText||cfg.profile.bioFallback||'Si tu es arrivé ici, c’est que tu partages la même passion que moi : la liberté, le homebrew et la communauté. Merci de ton soutien et bon surf !')}</p><span>— ${esc(cfg.home?.welcomeSignature||repo.owner||'evoX')}</span></div></section>
    <section class="approved-store panel">
      <div class="approved-head compact"><div><h2>▣ &nbsp;Store</h2><span>Accédez à tous mes outils et fichiers</span></div></div>
      <div class="approved-search">⌕ <input id="home-store-search" placeholder="Rechercher un fichier, un outil..."></div>
      <div class="approved-filters"><select><option>Toutes les catégories</option>${categoryNames.map(c=>`<option>${esc(c)}</option>`).join('')}</select><select><option>Toutes les sous-catégories</option></select><button id="home-reset">Réinitialiser</button></div>
      <div class="approved-categories">${categoryNames.map((c,i)=>`<a href="#store" class="approved-category"><b>${categoryIcons[i]}</b><span>${esc(c)}</span><small>(${countFor(c)||'0'})</small></a>`).join('')}</div>
    </section>
    <section class="approved-quick panel"><div class="approved-head compact"><div><h2>⇩ &nbsp;Ressources Rapides</h2></div></div><div class="approved-quick-grid">${aio.map(x=>`<a href="${esc(resolveUrl(x[2],repo))}" target="_blank" class="quick-item"><b>⇩</b><span><strong>Télécharger ${esc(x[0])}</strong><small>${esc(x[1])}</small></span></a>`).join('')}<a href="#docs" class="quick-item"><b>▤</b><span><strong>Documentation</strong><small>Guides & tutos</small></span></a><a href="#services" class="quick-item"><b>↗</b><span><strong>Services</strong><small>Mes services / redirections</small></span></a></div></section>
   </div>
 </div>
 <section class="approved-thanks"><span class="big-heart">♥</span><div><strong>${esc(cfg.home?.thanksTitle||"Un immense merci !")}</strong><small>${esc(cfg.home?.thanksText||"Merci à toutes les personnes qui me soutiennent, qui m'encouragent et qui font vivre cette communauté. Vous êtes géniaux !")}</small></div><em>${esc(cfg.home?.thanksSignature||"Keep the spirit alive ! ♡")}</em></section>
 <section class="approved-stats"><div><b>${esc(profile?.public_repos??'—')}</b><small>Dépôts GitHub</small></div><div><b>${storeData?.items?.length||'—'}</b><small>Fichiers disponibles</small></div><div><b>${new Set((storeData?.items||[]).map(x=>x.category).filter(Boolean)).size||'—'}</b><small>Catégories</small></div><div><b class="online">● Site en ligne</b><small>Détection automatique activée</small></div></section>${footer()}`;
 document.querySelectorAll('a[href="#news"],a[href="#store"],a[href="#docs"],a[href="#services"]').forEach(a=>a.onclick=e=>{e.preventDefault();state.page=a.getAttribute('href').slice(1);render()});
 document.getElementById('home-reset')?.addEventListener('click',()=>{const i=document.getElementById('home-store-search');if(i)i.value=''});
}
async function news(){
 setTitle("News"); if(!newsData)newsData=await readAllFeeds(cfg, cfg.news||{});
