...
async function home(){
 setTitle("Home");
 const latest=(newsData?.items||[]).slice(0,4);
 const aio=[
  ["Payloads AIO","Pack complet (.zip)","/archives/PS5_payloads_aio_latest.zip"],
  ["PKG AIO","Pack complet (.zip)","/archives/PS5_pkg_aio_latest.zip"],
  ["FFPFSC AIO","Pack complet (.zip)","/archives/PS5_ffpfsc_aio_latest.zip"],
  ["Apps AIO","Pack complet (.zip)","/archives/PS5_apps_aio_latest.zip"]
 ];
 ...
 const categoryNames=(cats.length?cats:["Payloads","Packages","FFPFSC","Applications","Tools","Divers"]).slice(0,6);
 const categoryIcons=["✦","◆","⚒","▣","⌘","◇"];
 content.innerHTML=`
 <section class="approved-hero">
   ...
   <div class="hero-copy">
     ...
     <div class="hero-actions">
       <a class="hero-chip" href="#store">✦ Payloads PS5</a><a class="hero-chip" href="#store">▣ Store</a><a class="hero-chip" href="#news">◔ News RSS</a><a class="hero-chip" href="#docs">▤ Documentation</a><a class="hero-chip" href="${esc(repo.githubUrl||'#')}" target="_blank">◈ Open Source</a>
     </div>
   </div>
 </section>
 <div class="approved-columns">
   ...
   <div class="approved-right">
    <section class="approved-welcome panel"><div class="welcome-art"></div><div class="approved-welcome-content"><h2>👋 Bienvenue !</h2><p>${esc(cfg.profile.bioFallback||'Si tu es arrivé ici, c’est que tu partages la même passion que moi : la liberté, le homebrew et la communauté. Merci de ton soutien et bon surf !')}</p><span>— ${esc(repo.owner||'evoX')}</span></div></section>
 ...
 <section class="approved-thanks"><span class="big-heart">♥</span><div><strong>Un immense merci !</strong><small>Merci à toutes les personnes qui me soutiennent, qui m'encouragent et qui font vivre cette communauté. Vous êtes géniaux !</small></div><em>Keep the spirit alive ! ♡</em></section>
 <section class="approved-stats"><div><b>${esc(profile?.public_repos??'—')}</b><small>Dépôts GitHub</small></div><div><b>${storeData?.items?.length||'—'}</b><small>Fichiers disponibles</small></div><div><b>${new Set((storeData?.items||[]).map(x=>x.category).filter(Boolean)).size||'—'}</b><small>Catégories</small></div><div><b class="online">● Site en ligne</b><small>Détection automatique activée</small></div></section>`;
